...
input$time_date <- "2026-07-03"
input$time_hour <- 11
input$raw_chamber <- "c_4_e1.5"
input$raw_date <- "2026-07-03"
```
...
plotlyOutput("plot_time", height = 480)
```
...
plotlyOutput("plot_raw", height = 480)
```
...
raw_comb <- read_csv(here::here("processed_data", "raw_comb.csv")) %>%
  mutate(used = ifelse(change_s >= 225 & change_s <= 515, "yes", "no"))
...
output$plot_time <- renderPlotly({
  raw_plot <- raw_comb %>%
    filter(
      as.Date(TIMESTAMP) == input$time_date,
