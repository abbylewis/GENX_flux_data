filter_old_data_2021 <- function(grouped_data) {
  if (is.null(grouped_data)) {
    ...
    return(setNames(
      data.frame(matrix(ncol = 17, nrow = 0)),
      c(
        "TIMESTAMP", "CH4d_ppm", "CO2d_ppm", "N2Od_ppb", "MIU_VALVE", "Manifold_Timer",
  ...
  sufficient_range <- correct_duration %>%
    filter(
      !is.na(CH4d_ppm),
      change_s > 60
    ) %>% # All chambers need to remove first 60 seconds
  ...
  peaks_raw <- sufficient_range %>%
    ...
    summarize(
      first_peak = change_s[which.max(CH4d_ppm)],
      .groups = "drop"
    ) %>%
  ...
  mins <- sufficient_range %>%
    ...
    # Find min
    summarize(
      first_min = change_s[which.min(CH4d_ppm)],
  ...
  second_peaks_raw <- sufficient_range %>%
    ...
    # Find max
    summarize(
      second_peak = change_s[which.max(CH4d_ppm)],
      .groups = "drop"
    ) %>%
    filter( # Doesn't count as a peak if it's at the end
  ...
  second_mins <- sufficient_range %>%
    ...
    # Find min
    summarize(
      second_min = change_s[which.min(CH4d_ppm)],
  ...
  combined_cutoff <- grouped_data %>%
    ...
    mutate(
      comb_peak = ifelse(is.na(second_peak), first_peak, second_peak),
  ...
  cutoff_daily <- combined_cutoff %>%
    ...
    summarize(
      cutoff = density(comb_min, bw = 10)$x[which.max(density(comb_min, bw = 10)$y)],
      cutoff = round(cutoff),
      .groups = "drop"
    )
