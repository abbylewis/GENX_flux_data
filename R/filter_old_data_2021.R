filter_old_data_2021 <- function(grouped_data) {
  ...
  peaks_raw <- grouped_data %>%
    ...
    summarize(
      max_s = unique(max_s), .groups = "drop",
    ...
    summarize(
      cutoff = density(max_s, bw = 10)$x[which.max(density(max_s, bw = 10)$y)],
      cutoff = round(cutoff) + buffer,
      .groups = "drop"
    )

  write_csv(peaks_raw, here::here("processed_data", "peaks_raw_2021.csv"))
