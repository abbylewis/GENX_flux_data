...
download_new_data <- function(lgr_folder = here::here("Raw_data", "dropbox_downloads")) {
  ...
  relevant_files <- drop_dir(path = "GCREW_LOGGERNET_DATA/archive_data") %>%
    filter(grepl("GENX_INSTRUMENT_FLUX_COMB", name) |
      ...
      grepl(
        "GENX_INSTRUMENT_FLUX_LGR1_20240417020048.dat|GENX_INSTRUMENT_FLUX_LGR1_20240403020047.dat|GENX_INSTRUMENT_FLUX_LGR1_20240501020050.dat",
        name
      ), )
  current <- drop_dir(path = "GCREW_LOGGERNET_DATA/current_data") %>%
    filter(
      grepl("GENX_INSTRUMENT_FLUX_COMB", name),
      !grepl("backup", name)
    )
...
download_new_data_error_codes <- function(lgr_folder = here::here("Raw_data", "with_error_codes")) {
  # Identify all files
  # GENX flux vs GENX LGR in 2021
  message("Looking for new data files on dropbox")
  relevant_files <- drop_dir(path = "GCREW_LOGGERNET_DATA/archive_data") %>%
    filter(grepl("GENX_INSTRUMENT_FLUX_7810", name))
  current <- drop_dir(path = "GCREW_LOGGERNET_DATA/current_data") %>%
    filter(
      grepl("GENX_INSTRUMENT_FLUX_7810", name),
      !grepl("backup", name)
    )
