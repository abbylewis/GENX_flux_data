...
checkboxGroupInput("years", "Years to plot",
  ...
  selected = c("2021", "2022", "2023", "2024", "2025")
)
```
...
# slopes_2021 <- read_csv("data/L0_2021_best_fits.csv", show_col_types = F) %>%
#  mutate(year = year(TIMESTAMP),
#         yday = yday(TIMESTAMP))
#
# slopes_2025 <- read_csv("data/L0_for_dashboard.csv", show_col_types = F) %>%
#  mutate(year = year(TIMESTAMP),
#         yday = yday(TIMESTAMP))
#
...
slopes_l0 <- read_csv("data/L0_bestfits_all.csv", show_col_types = F) %>%
  mutate(
    year = year(TIMESTAMP),
    yday = yday(TIMESTAMP)
  )
...
output$plot1 <- renderPlotly({
  ...
  p1 <- slopes_recent2 %>%
    mutate(
      chamber = factor(chamber, labels = chamber_levels),
      R2 = round(R2, 2)
    ) %>%
