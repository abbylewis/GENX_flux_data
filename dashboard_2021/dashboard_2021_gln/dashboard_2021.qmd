...
slopes_l0 <- read_csv(here::here("raw_data", "2021-05-05_2021-12-13_CH4_CO2_fluxes (Jan 2025).csv"), show_col_types = F) %>%
  mutate(
    TIMESTAMP = as_datetime(TIMESTAMP, format = "%m/%d/%Y %H:%M"),
    year = year(TIMESTAMP),
    yday = yday(TIMESTAMP)
  )
...
output$plot1 <- renderPlotly({
  slopes <- slopes_l0 %>%
    ...
    select(-CH4_slope_umol_per_day, -CO2_slope_mmol_per_day)
