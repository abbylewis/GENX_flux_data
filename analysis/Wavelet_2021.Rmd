...
target <- read_csv("../processed_data/L0_bestfits_all.csv") %>%
  mutate(TIMESTAMP = as_datetime(TIMESTAMP)) %>%
  filter(year(TIMESTAMP) == 2021) %>%
  rename(CH4 = CH4_slope_ppm_per_day)
...
dfs %>%
  ...
  filter(as.Date(TIMESTAMP) %in% seq.Date(as.Date("2021-08-27"), as.Date("2021-09-06"), by = "1 day"),
         name == "33 hours", 
...
to_plot <- discrete_scales %>%
  ungroup() %>%
  select(TIMESTAMP, grouped_scale, MIU_VALVE, CH4) %>%
  mutate(TIMESTAMP = round_date(TIMESTAMP, "2 hours")) %>%
  pivot_wider(names_from = MIU_VALVE, values_from = CH4) %>%
...
discrete_scales %>%
  ...
  filter(as.Date(TIMESTAMP) %in% seq.Date(as.Date("2025-03-27"), as.Date("2025-04-06"), by = "1 day"),
         grouped_scale == "hourly",
         name %in% c("CH4", "Depth_cm", "GPP")) %>%
