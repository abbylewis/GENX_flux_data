...
ch4 <- read_csv(here::here("processed_data","L2- partitioned_and_gap_filled.csv")) %>%
  filter(!TIMESTAMP == as_datetime("2025-08-20 18:10:40"))
...
dfs %>%
  ...
  filter(as.Date(TIMESTAMP) %in% seq.Date(as.Date("2025-08-01"), as.Date("2025-10-04"), by = "1 day"),
         name == "4 hours", 
         var_name %in% c("CH4_filled", "Depth_cm")) %>%
  pivot_wider(names_from = var_name, values_from = value) %>%
  ggplot(aes(x = CH4_filled, y = Depth_cm))+
  geom_point()+
...
grouped_pct_var %>%
  mutate(MIU_VALVE = factor(MIU_VALVE, 
                            levels = 1:12,
                            labels = chamber_levels2)) %>%
  ggplot(aes(color = MIU_VALVE, y = var, 
             x = grouped_scale, group = MIU_VALVE)) +
  geom_point()+
  geom_line()+
...
grouped_pct_var %>%
  ...
  ggplot(aes(x = MIU_VALVE, y = var, group = grouped_scale)) +
...
mut_infs <- discrete_scales %>%
  ...
  summarize(entropy_y = entropy(bin_ranked(CH4_filled, N_BINS)),
            mut_inf = MutInf(
              bin_ranked(exp_var_value, N_BINS), 
              bin_ranked(CH4_filled, N_BINS)
              ) / 
              entropy_y) %>%
