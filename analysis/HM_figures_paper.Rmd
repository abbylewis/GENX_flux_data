...
annual <- hot_moments %>%
  ...
  ggplot(aes(x = Temp_C, y = hot_sum/sum))+
  ...
  ylab("Percent of emissions that\ncome from the top 5% of fluxes")+
...
# INTERESTING MONTHLY PATTERN

hot_moments_monthly <- hm_df %>%
  mutate(month = month(DateTime)) %>%
  group_by(MIU_VALVE, month) %>%
  mutate(hot_moment_quant = ifelse(CH4_umol_m2_h > 
                                     quantile(CH4_umol_m2_h, 
                                              probs = 0.95, na.rm = TRUE),
                                   "hot", "not"),
...
monthly <- hot_moments_monthly %>%
  ggplot(aes(x = month, y = hot_sum/sum))+
  geom_point(aes(color = MIU_VALVE), alpha = 0.5, shape = 21)+
  scale_color_manual(values = color.gradient, name = "Chamber")+
  scale_fill_manual(values = color.gradient, name = "Chamber")+
  geom_smooth(aes(color = MIU_VALVE), se = F, 
              method = "gam",
              formula = y ~s(x, bs = "cs", fx = T, k = 5))+
  egg::theme_article()+
  ylab("Percent of emissions that\ncome from the top 5% of fluxes")+
  xlab("Month")+
  theme(panel.grid.major = element_line(color = "grey93", linewidth = 0.5),
...
ggsave(monthly, filename = "../figures/Figure 3 - HM monthly.jpg", dpi = 300, width = 3.5, height = 4.5)

patchwork::wrap_plots(annual + theme(legend.position = "none"), 
                      monthly + theme(legend.position = "none"), 
                      axes = "collect", guides = "collect", design = "ABB") &
  theme(legend.position = "bottom")
```
