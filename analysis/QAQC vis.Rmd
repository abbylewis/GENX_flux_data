...
slopes_l0 <- read_csv(here::here("processed_data", "L0.csv"), show_col_types = F) %>%
  ...
  mutate(
    CH4_nrmse = CH4_rmse / (CH4_max - CH4_min),
...
pct <- slopes_filt %>%
  ...
  summarise(
    pct10 = sum(abs(CO2_slope_ppm_per_day) < 10) / n() * 100,
...
removed <- slopes_filt %>%
  filter(!is.na(CO2_slope_ppm_per_day)) %>%
  mutate(
    CO2_ppm_10 = abs(CO2_slope_ppm_per_day) < 10,
...
open <- temp_difs %>%
  ...
  group_by(miu_valve_bme) %>%
  mutate(
    temp_change_thresh = ifelse(temp_change_quantile < 0.8, T, F),
...
open_final <- temp_difs %>%
  filter(miu_valve_bme == miu_valve_flux) %>%
  mutate(
    open = ifelse(temp_change_quantile < 0.8, T, F),
...
both <- slopes %>%
  ...
  geom_point() +
  guides(
    alpha = guide_legend(title = "Flux > 10000\numol per day"),
