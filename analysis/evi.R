...
# SET DATES (note script is designed for 2026 only)
...
formatted <- pred_df_beck %>%
  ...
  rename(evi_observed = evi,
         evi_predicted = pred) %>%
  mutate(Date_mod = as.Date(doy - 1, origin = "2026-01-01"))
...
evi_2025 <- read_csv(here::here("processed_data","evi.csv")) %>%
  filter(Date < "2026-02-15")
out <- bind_rows(final, evi_2025)
write_csv(out, here::here("processed_data","EVI_2026.csv"))
