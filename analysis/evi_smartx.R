...
# SET DATES (note script is NOT designed for the current year)
...
roi <- terra::vect(here::here("Raw_data","smartx_c3.geojson"))
...
write_csv(final, here::here("processed_data", paste0("SMARTX_EVI_",year,".csv")))

#### PLOT ####
list.files(here::here("processed_data"), pattern = "SMARTX_EVI", full.names = T) %>%
  map(read_csv) %>%
  bind_rows() %>%
  mutate(year = as.factor(year(Date)),
         doy = yday(Date)) %>%
  ggplot(aes(x = doy, color = year)) +
  geom_point(aes(y = evi_observed)) +
  geom_line(aes(y = evi_predicted), data = . %>% filter(!year == 2026)) +
  scale_color_manual(values = c("grey90", "grey80", "grey70", 
                                "grey60", "grey50", "blue", "red"))+
  theme_minimal() +
  labs(title = "EVI and Double Log Fit (Beck Method)",
       x = "Day of Year",
       y = "EVI")
