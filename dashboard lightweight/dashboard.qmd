...
output$plot_flux <- renderPlotly({
  ...
  part_recent2 <- part_recent %>%
    ...
    select(c(CH4, N2O, NEE, H2O, DateTime, Chamber)) %>%
    pivot_longer(matches("CH4|N2O|NEE|H2O"),
      names_to = "gas"
    ) %>%
    mutate(gas = gas %>%
             replace_values(
...
output$plot_co2 <- renderPlotly({
  ...
  part_recent2 <- part_recent %>%
    rename(Chamber = MIU_VALVE) %>%
    select(c(NEE, GPP, Reco, H2O, DateTime, Chamber)) %>%
...
output$plot_raw <- renderPlotly({
  raw_plot <- raw %>%
    ...
    pivot_longer(matches("CH4d_ppm|N2Od_ppm|CO2d_ppm|H2O_ppm"),
      names_to = "gas"
    ) %>%
    mutate(
      gas = case_match(gas,
