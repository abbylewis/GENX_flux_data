...
output$plot_forecast <- renderPlotly({
  ...
  p1 <- forecast %>%
    mutate(site_id = factor(site_id, levels = 1:12,
                            labels = chamber_levels)) %>%
    pivot_wider(names_from = "parameter", values_from = "prediction") %>%
    ggplot(aes(x = datetime, color = site_id)) +
    geom_hline(yintercept = 0, color = "grey50") +
    geom_vline(xintercept = input$today) +
    geom_line(aes(y = mu)) +
    geom_ribbon(aes(ymin = mu - sigma, ymax = mu + sigma, fill = site_id), alpha = 0.3) +
    geom_point(data = part_recent2 %>%
                 mutate(site_id = factor(chamber, levels = 1:12,
                                         labels = chamber_levels)), 
               aes(x = DateTime, y = value, 
                   alpha = DateTime >= input$today)) +
    geom_line(data = part_recent2 %>%
                mutate(site_id = factor(chamber, levels = 1:12,
                                        labels = chamber_levels)) %>%
                filter(DateTime <= input$today), 
               aes(x = DateTime, y = value)) +
    ...
    #facet_wrap(~site_id) +
    ggtitle(paste0("CH4 forecasts for ", input$today))
