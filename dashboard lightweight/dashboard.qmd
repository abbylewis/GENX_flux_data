...
output$plot_flux <- renderPlotly({
  p1 <- plot_flux(partitioned, 
                  compare_years = input$compare_years, 
                  today = input$today, 
                  daily = input$daily,
                  days_to_plot = input$days_to_plot, 
                  gases = input$gases, 
                  smooth = input$smooth,
                  chamber_levels = chamber_levels, 
...
################ SOIL TEMP ####################
soil_temp <- read_csv("https://raw.githubusercontent.com/abbylewis/GENX_flux_data/refs/heads/master/processed_data/soil_temp_hourly.csv")
output$plot_soil_temp <- renderPlotly({
  if(input$compare_years == "Compare years"){
    st_recent <- soil_temp %>%
      filter(
        yday(as_date(TIMESTAMP)) <= yday(input$today),
        yday(as_date(TIMESTAMP)) > (yday(input$today) - input$days_to_plot)
      ) %>%
      mutate(year = year(TIMESTAMP))
  } else {
    st_recent <- soil_temp %>%
  ...
  if (input$daily == "Daily mean") {
    st_recent2 <- st_recent %>%
      mutate(
        TIMESTAMP = as_date(TIMESTAMP),
        TIMESTAMP = as.POSIXct(TIMESTAMP)
      ) %>%
      group_by(TIMESTAMP, Chamber, year) %>%
      summarize(
        SoilTemp_25cm_C = mean(SoilTemp_25cm_C, na.rm = T),
        .groups = "drop"
      )
  } else {
    st_recent2 <- st_recent
  ...
  p1 <- st_recent2 %>%
    ...
    ggplot(aes(x = TIMESTAMP, y = SoilTemp_25cm_C, color = Chamber)) +
    geom_point(size = 0.5) +
    {
      if (input$smooth == "Smoothed") {
        geom_smooth(se = FALSE, method = "gam")
