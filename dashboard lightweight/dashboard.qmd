...
slopes_l0 <- read_csv("https://raw.githubusercontent.com/abbylewis/GENX_flux_data/refs/heads/master/processed_data/L0_for_dashboard.csv", show_col_types = F) 
```
...
dateInput('today', 'Today', 
          min = "2025-03-18", max = as.Date(max(slopes_l0$TIMESTAMP)), 
          value = as.Date(max(slopes_l0$TIMESTAMP)))
br()
sliderInput('days_to_plot', 'Days of historical data to plot', 
            0, 
            round(as.numeric(difftime(as.Date(max(slopes_l0$TIMESTAMP)), "2025-03-18", units = "days"))),
            value = 5)
br()
