...
hot_moments %>%
  ...
  ggplot(aes(x = temp, y = tot_hot/tot))+
  ...
  ggtitle("What percent of emissions come from the top 5% of fluxes?")+
  theme_bw()
...
hot_real_data %>%
  ...
  geom_col()+
  geom_col(aes(x = tot_hot), fill = "black", alpha = 0.5)+
...
df_out_daytime <- df %>%
  ...
  summarize(flux = calc_mean_flux(CH4_slope_ppm_per_day, unique(days), 1))
