...
hot_moments <- slopes_l0 %>%
  group_by(MIU_VALVE) %>%
  mutate(
    hot_moment_sd = ifelse(CH4_slope_ppm_per_day >
...
output$sim_plot <- renderPlotly({
  ...
  df <- slopes_l0 %>%
    ...
    mutate(
      chamber = factor(MIU_VALVE, levels = 1:12, labels = chamber_levels),
      week = week(date)
    )
